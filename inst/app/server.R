shinyServer(
  ...
  function(input, output) {
    ...
    velg_level1 <- c(levels(factor(healthatlas_data$level1)))

    velg_level2 <- eventReactive(input$level1,{
      tmpdata <- dplyr::filter(healthatlas_data, level1 == input$level1)
      level2 <- c(levels(factor(tmpdata$level2)))
      return(level2)
    })

    velg_level3 <- eventReactive(c(input$level1, input$level2), {
